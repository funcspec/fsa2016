...
evalc (Cj cs) env = all (`evalc` env) cs
evalc (Dj cs) env = any (`evalc` env) cs
...
exec wh@(While c s) env =
 if not (evalc c env) then env
 else exec wh (exec s env)
...
fibon :: (Integer, Integer, Integer) -> Integer
fibon = whiler
         (\ (_,_,n) -> n > 0)
         (\ (x,y,n) -> (y,x+y,n-1))
         (\ (x,_,_) -> x)
